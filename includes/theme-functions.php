...
- Woo Portfolio, add support for the meta manager.
...
- Get a menu name
- Construct the query arguments for the "Magazine" grid
-------------------------------------------------------------------------------------*/
	...
	function woo_pagenav( $custom_query = '' ) {
		...
		?>
            <div class="nav-entries">
                <?php next_posts_link( '<span class="nav-prev fl"><i class="fa fa-angle-' . esc_attr( $left_class ) . '"></i> '. __( 'Older posts', 'woothemes' ) . '</span>', $max_pages ); ?>
                <?php previous_posts_link( '<span class="nav-next fr">'. __( 'Newer posts', 'woothemes' ) . ' <i class="fa fa-angle-' . esc_attr( $right_class ) . '"></i></span>' ); ?>
                <div class="fix"></div>
            </div>
		<?php
			} // End IF Statement
...
if ( ! function_exists( 'woo_add_slides' ) ) {
	function woo_add_slides() {
		...
		$labels = array(
			...
			'singular_name' => _x( 'Slide Group', 'taxonomy singular name', 'woothemes' ),
...
/*-----------------------------------------------------------------------------------*/
/* Returns true if there are any items with the 'portfolio' post type */
/*-----------------------------------------------------------------------------------*/
if ( ! function_exists( 'woo_has_portfolio_items' ) ) {
	function woo_has_portfolio_items() {
		$has_portfolio = get_posts( 'post_type=portfolio' );
		return ( $has_portfolio ? true : false );
	}
}
...
if ( ! function_exists( 'woo_add_portfolio' ) ) {
	function woo_add_portfolio() {
		...
		$icon = get_template_directory_uri() .'/includes/images/portfolio.png';
		if ( '3.9' <= $wp_version ) {
...
add_filter( 'woo_meta_manager_meta_areas', 'woo_portfolio_add_meta_manager_data' );

function woo_portfolio_add_meta_manager_data ( $meta_areas ) {
	$maybe_disable_portfolio = get_option( 'woo_portfolio_disable', 'false' );
	// Sanity check.
	if ( 'true' == $maybe_disable_portfolio ) { return $meta_areas; }

	$meta_areas['portfolio_meta'] = array(
		'woo_filter_portfolio_post_meta' => array(
				'title' => __( 'Above portfolio item content', 'woothemes' ),
				'default' => '<span class="small">' . __( 'By', 'woothemes' ) . '</span> [post_author_posts_link] <span class="small">' . __( 'on', 'woothemes' ) . '</span> [post_date] <span class="small">' . __( 'in', 'woothemes' ) . '</span> [post_categories before="" taxonomy="portfolio-gallery"]',
				'stored_value' => '',
				'description' => __( 'Data above the content of your portfolio items.', 'woothemes' )
			)
	);

	return $meta_areas;
} // End woo_portfolio_add_meta_manager_data()
