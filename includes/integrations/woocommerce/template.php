 ...
 * @return void
 */
if ( ! function_exists( 'woo_nav_cart_contents_link' ) ) {
function woo_nav_cart_contents_link () {
...
?>
	<a class="cart-contents" href="<?php echo esc_url( WC()->cart->get_cart_url() ); ?>" title="<?php _e( 'View your shopping cart', 'woothemes' ); ?>">
		...
		<span class="count"><?php echo wp_kses_data( sprintf( _n( '%d item', '%d items', WC()->cart->get_cart_contents_count(), 'woothemes' ), WC()->cart->get_cart_contents_count() ) );?></span> - <?php echo wp_kses_data( WC()->cart->get_cart_subtotal() ); ?>
		<?php } ?>
	</a>
<?php
} // End woo_nav_cart_contents_link()
}
...
function woo_add_nav_cart_link () {
	...
	if ( is_woocommerce_activated() && 'true' == $settings['header_cart_link'] ) { ?>
    	<ul class="<?php echo esc_attr( $class ); ?>">
    		<li>
    			<?php woo_nav_cart_contents_link(); ?>
    			<ul>
...
function woocommerce_prettyphoto() {
	global $woo_options;
	if ( isset($woo_options[ 'woo_enable_lightbox' ]) && $woo_options[ 'woo_enable_lightbox' ] == "true" && is_product() ) {
		?>
			<script>
