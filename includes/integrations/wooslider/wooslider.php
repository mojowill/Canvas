...
if ( ! function_exists( 'woo_custom_image_anchor_attributes' ) ) {
	/**
	 * woo_custom_image_anchor_attributes adds rel attribute to existing image anchor tags
	 * @since 5.11.5
	 * @return string html markup
	 */
	function woo_custom_image_anchor_attributes() {
		global $woo_options;
		$attr_value = '';
		if ( isset($woo_options[ 'woo_enable_lightbox' ]) && $woo_options[ 'woo_enable_lightbox' ] == "true" ) {
			$attr_value = 'rel="lightbox"';
		}
		return $attr_value;
	}
	add_filter( 'wooslider_slide_carousel_slides_attr', 'woo_custom_image_anchor_attributes' );
	/* 	Other Available filters
	*	wooslider_slide_carousel_slides_attr
	* 	wooslider_slide_post_slides_attr
	*/
}

if ( ! function_exists( 'woo_custom_start_callback_attributes' ) ) {
	/**
	 * woo_custom_start_callback_attributes adds attributes to the start callback when flexslider loads
	 * @since 5.11.5
	 * @return string javascript markup
	 */
	function woo_custom_start_callback_attributes() {
		global $woo_options;
		$testvalue = '';
		if ( isset($woo_options[ 'woo_enable_lightbox' ]) && $woo_options[ 'woo_enable_lightbox' ] == "true" ) {
			$testvalue = "jQuery( 'a[rel^=\"lightbox\"]' ).prettyPhoto({social_tools: false});";
		}
		return $testvalue;
	}
	add_filter( 'wooslider_callback_start_appended', 'woo_custom_start_callback_attributes' );
}
