...
function woo_shortcode_post_comments ( $atts ) {
	...
	if ( ( get_option( 'woo_comments' ) === 'none' || ! comments_open() ) && $atts['hide_if_off'] === 'enabled' )
		return;
...
function woo_shortcode_footer_woothemes_link ( $atts ) {
	...
	$output = sprintf( '%s<a href="%s" alt="%s" title="%s">%s</a>%s', $atts['before'], esc_url( $woo_link ), 'Premium WordPress Themes & Plugins by WooThemes' , 'Premium WordPress Themes & Plugins by WooThemes', '<img src="' . esc_url( get_template_directory_uri() . '/images/woothemes.png' ) . '" width="74" height="19" alt="WooThemes" />', $atts['after'] );
